...
def calculate_tensor_size(tensor_shape: tuple, dtype: np.dtype) -> float:
    """
    Calculate the size of a tensor node in MB.
    Args:
        tensor_shape (tuple): Shape of the tensor.
        dtype (np.dtype): Data type of the tensor.
    Returns:
        float: Size of the tensor node in MB.
    """
    num_elements = np.prod(tensor_shape)
    element_size = dtype.itemsize
    size_in_mb = (num_elements * element_size) / (1024 * 1024)
    return size_in_mb
...
def should_quantize(node: torch.fx.Node) -> bool:
    ...
    shape, dtype = node.meta["val"].shape, node.meta["val"].dtype
    size_in_mb = calculate_tensor_size(shape, dtype)
        ...
        if node.meta.get("saved_for_quantization", False):
            ...
            if torch._inductor.config.post_grad_fusion_options[
                ...
                with graph.inserting_after(node):
                    ...
                    scale_node = next(
                        bwd_input for bwd_input in bw_inputs if bwd_input.name == scale_name
                    )
                ...
                with graph.inserting_after(divided_target_node_32):
                    ...
                    dequant_node.meta["val"] = torch.ops.prims.convert_element_type.default(divided_target_node_32.meta["val"], dequant_type)
                    dequant_node.meta["tensor_meta"] = extract_tensor_metadata(dequant_node.meta["val"])
            else:
                with graph.inserting_after(node):
                    dequant_node = graph.call_function(
                        torch.ops.prims.convert_element_type.default,
